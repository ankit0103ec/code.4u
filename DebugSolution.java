...
public class DebugSolution {
    ...
    public static int get_ans(int N, List<Integer> A) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = i + 1; j < N; j++) {
                int beautyVal = beauty(A.get(i), A.get(j));
                System.out.println("beauty(" + A.get(i) + ", " + A.get(j) + ") = " + beautyVal);
                result = (result + beautyVal) % MOD;
            }
        }
    ...
    private static int beauty(int x, int y) {
        ...
        System.out.println("Divisors of " + x + ": " + divisorsX);
        System.out.println("Divisors of " + y + ": " + divisorsY);
